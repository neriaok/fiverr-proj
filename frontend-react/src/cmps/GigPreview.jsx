  ...
  return (
    <div className="preview">
      ...
      {isImage(gig.imgUrls[currentImageIndex]) ? (
        ...
        <label className="prev-btn" onClick={handlePrev}>{levelService.leftSign}</label>
        <label className="next-btn" onClick={handleNext}>{levelService.rightSign}</label>
      </div>
      <div className="gig-info">
        <div className="owner-info">
          <img className='owner-img' src={gig.owner.imgUrl} alt="" />
          <p className='ad-by'>Ad by <span className="owner-name">{gig.owner.fullname}</span></p>
          <p className={ownerLevel}>{gig.owner.level}{ownerLevelIcon}</p>
        </div>
